...
public class PlayerController : MonoBehaviour {
	...
	void Update () {
		...
		this.transform.Translate (new Vector3 (inputZ  * Time.deltaTime * velocity, 0, velocity * Time.deltaTime)); // Moving the player
		...
		if (car.transform.position.x < -1.25f) {
			car.transform.position = new Vector3 (-1.25f, car.transform.position.y, car.transform.position.z);
		}
